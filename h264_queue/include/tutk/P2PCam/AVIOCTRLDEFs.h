...
typedef enum 
{
	...
	IOTYPE_USER_IPCAM_PTZ_COMMAND_ACK 			= 0x1002, 	// P2P PTZ Command Msg ACK
	#endif
	...
	//defined by xiaomi
	IOTYPE_USER_IPCAM_PLAYBACK_CTRL_REQ         = 0xF001,
	IOTYPE_USER_IPCAM_PLAYBACK_CTRL_RESP        = 0xF002,
...
typedef struct
{
    ...
    unsigned char angle;   // PTZ control angle  // if 0 small, 1 big
    ...
    unsigned char istoend; // whether is moved to end
    unsigned char reserve;
    int sessionid; // a random id (must be unique)
    int avindex;
} SMsgAVIoctrlPtzCmd;
